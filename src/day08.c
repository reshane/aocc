...
// day 8
long long d8p1_solver(char *input, const size_t input_sz,
    ...
    for (size_t idx = 0; idx < iterations; ++idx) {
        ...
        if (groups[a]) {
            // merge into a
            // set all indices in groups to groups[a] where groups[i] =
            // groups[b];
            size_t new = groups[a];
            size_t old = groups[b];
            if (old) {
    ...
    qsort((void *)&group_cts, ((D8_MAX_LNS * (D8_MAX_LNS - 1)) / 2) - 1,
          sizeof(group_cts[0]), &comp_szt);
    long long total = 1;
