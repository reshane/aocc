...
uint32_t hash(StringView *node_str) {
    ...
    //     h |= node_str->buf[i];
    // }
    // uint32_t h = ((uint32_t)0xFFFFFF00 & *(uint32_t*)((void*)node_str->buf))
    // >> 8; uint32_t h = (*(uint32_t*)(node_str->buf)) >> 8; printf("[%p %p %p]
    // -> %"PRIu32"\n", uint32_t h = ((((uint32_t)(uint8_t)node_str->buf[0]) <<
    // 16
    //                | ((uint32_t)(uint8_t)node_str->buf[1]) << 8
    //                | ((uint32_t)(uint8_t)node_str->buf[2])) - 0x616161) %
    //                D11_MAX_LNS;
...
uint32_t get_index(StringView *node_str, StringView *nodes, int *populated) {
    ...
    if (populated[node_hash]) {
        while (sv_cmp(node_str, &nodes[node_hash]) != 0 &&
               node_hash < D11_MAX_LNS) {
...
uint32_t d11_set_insert(d11set *set, StringView *node_str) {
    ...
    if (set->populated[node_hash]) {
        while (sv_cmp(node_str, &set->nodes[node_hash]) != 0 &&
               node_hash < D11_MAX_LNS) {
            // printf("Nodes collide! ["); sv_print(&node_str); printf("] & [");
            // sv_print(&node_strs[node_hash]); printf("] -> %"PRIu32"\n",
            // node_hash);
...
long long d11_dist(uint32_t start, uint32_t end,
                   uint32_t nbors[D11_MAX_LNS][D11_MAX_LNS], size_t *nbor_cts) {
    ...
    while (queue.size != 0) {
        ...
        // printf("curr: %"PRIu32" -> ", curr.data);
        // sv_print(&node_strs[curr.data]); printf("\n");
...
long long d11_solve_p1(char *input, const size_t input_sz) {
    ...
    while (lines(&split)) {
        ...
        while (delim(&ln_split, ' ')) {
            ...
            uint32_t nbor_idx =
                get_index(&nbor_str, &node_strs[0], &populated[0]);
            // printf("["); sv_print(&nbor_str); printf("](%"PRIu32") ",
            // nbor_hash);
...
long long paths(uint32_t src, uint32_t dst, uint32_t *po, size_t pol,
                uint32_t nbors[D11_MAX_LNS][D11_MAX_LNS],
                size_t nbor_cts[D11_MAX_LNS]) {
