...
long long d9_solve_p2(char *input, const size_t input_sz) {
    ...
    while (lines(&split)) {
        ...
        for (size_t idx = 0; idx < lns_ct; ++idx) {
            areas[areas_ct++] = (p9e){
                .a = idx, .b = lns_ct, .w = area(&pts[idx], &pts[lns_ct])};
        }
        lns_ct++;
    }
    for (size_t idx = 0; idx < lns_ct; ++idx) {
        ...
        Point prev = pts[idx];
        Point dir =
            (Point){.x = comp_ll(prev.x, curr.x), .y = comp_ll(prev.y, curr.y)};
        do {
            // update, add
            prev.x += dir.x;
            prev.y += dir.y;
            assert("perim has run out of space!" &&
                   perimct < D9_MAX_LNS * D9_MAX_LNS);
            perim[perimct++] = prev;
    ...
    for (size_t idx = 0; idx < areas_ct; ++idx) {
        ...
        for (size_t p = 0; p < perimct; ++p) {
            ...
            if (perim[p].x > x1 && perim[p].x < x2 && perim[p].y > y1 &&
                perim[p].y < y2) {
                valid = 0;
                break;
            }
