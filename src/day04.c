...
void d4_parse_input(char *input, size_t input_sz, StringView *d4_lines,
                    int *d4_ct) {
...
long long d4_count(StringView *d4_lines, int d4_ct, Point *d4_pts,
                   int *d4_pts_head) {
    for (int l = 0; l < d4_ct; ++l) {
        for (int c = 0; c < (int)d4_lines[l].len; ++c) {
            int nbors = 0;
            if (l > 0) {
                if (d4_lines[l - 1].buf[c] == '@')
                    nbors++;
                if (c > 0 && d4_lines[l - 1].buf[c - 1] == '@')
                    nbors++;
                if (c < (int)d4_lines[l - 1].len - 1 &&
                    d4_lines[l - 1].buf[c + 1] == '@')
                    nbors++;
            }
            if (l < d4_ct - 1) {
                if (d4_lines[l + 1].buf[c] == '@')
            ...
            if (c < (int)d4_lines[l].len - 1 && d4_lines[l].buf[c + 1] == '@')
                nbors++;
            if (c > 0 && (int)d4_lines[l].buf[c - 1] == '@')
                nbors++;

            if (nbors < 4 && d4_lines[l].buf[c] == '@') {
                assert("d4_pts ran out of space" &&
                       (*d4_pts_head) + 1 < D4_PTS_MAX);
...
long long d4_solve_p1(char *input, size_t input_sz) {
    ...
    return d4_count((StringView *)&d4_lines, d4_ct, (Point *)&d4_pts,
                    &d4_pts_head);
}
...
long long d4_solve_p2(char *input, size_t input_sz) {
    ...
    long long total = 0;
    d4_parse_input(input, input_sz, (StringView *)&d4_lines, &d4_ct);
    while (d4_count((StringView *)&d4_lines, d4_ct, (Point *)&d4_pts,
                    &d4_pts_head) > 0) {
        total += d4_pts_head;
        do {
