...
int aoc_slurp_file(const char *file_name, char *input, size_t *input_sz) {
    ...
    if (num_bytes > BUF_CAP) {
        fprintf(stderr,
                "Could not fit input (%" PRIu64
                " bytes) into input buffer (%d bytes)\n",
...
void aoc_fill_input(const char *input_str, char *input, size_t *input_sz) {
    ...
    if (input_len > BUF_CAP) {
        fprintf(
            stderr,
            "Could not fit input (%zu bytes) into input buffer (%d bytes)\n",
            input_len, BUF_CAP);
        return;
    }
...
void sv_print(StringView *sv) { printf("%.*s", (int)sv->len, sv->buf); }
...
int delim(Splitter *split, char c) {
    split->st += split->sz;
    if (split->buf[split->st] == c)
        split->st++;
    split->sz = 0;
    while (split->st + split->sz <= split->mx &&
           split->buf[split->st + split->sz] != c) {
        split->sz++;
    }
    return split->st < split->mx;
}
...
void Map_insert(Map *map, Key *k, void *v) {
    ...
    if (map->size == map->cap) {
        map->cap *= 2;
        Entry *newMem =
            (Entry *)realloc(map->entries, map->cap * sizeof(Entry));
        if (!newMem) {
...
size_t Vector_i_size(Vector_i *v) { return v->size; }

size_t Vector_i_capacity(Vector_i *v) { return v->capacity; }
...
size_t Vector_f_size(Vector_f *v) { return v->size; }

size_t Vector_f_capacity(Vector_f *v) { return v->capacity; }
...
size_t Matrix_i_rows(Matrix_i *m) { return m->rows; }

size_t Matrix_i_cols(Matrix_i *m) { return m->cols; }
...
size_t Matrix_u32_rows(Matrix_u32 *m) { return m->rows; }

size_t Matrix_u32_cols(Matrix_u32 *m) { return m->cols; }
...
size_t Matrix_f_rows(Matrix_f *m) { return m->rows; }

size_t Matrix_f_cols(Matrix_f *m) { return m->cols; }
